...
class GradientBackground {
  constructor(sceneManager) {
    ...
    this.uniforms = {
      ...
      // Default palette (scheme 1): from user-provided hex array
      // Color 1: #050505
      // Color 2: #000000
      // Color 3: #D2E449
      // Color 4: #23240F
      // Color 5: #363530
      // Color 6: #000000
      uColor1: { value: new THREE.Vector3(0.02, 0.02, 0.02) },   // #050505
      uColor2: { value: new THREE.Vector3(0.0, 0.0, 0.0) },      // #000000
      uColor3: { value: new THREE.Vector3(0.824, 0.894, 0.286) },// #D2E449
      uColor4: { value: new THREE.Vector3(0.137, 0.141, 0.059) },// #23240F
      uColor5: { value: new THREE.Vector3(0.212, 0.208, 0.188) },// #363530
      uColor6: { value: new THREE.Vector3(0.0, 0.0, 0.0) },      // #000000
      uSpeed: { value: 1.2 }, // Faster animation
      uIntensity: { value: 1.8 },
      uTouchTexture: { value: null },
      uGrainIntensity: { value: 0.08 },
      uZoom: { value: 1.0 }, // Zoom/scale control - lower = less zoomed (more visible)
      // Base background tone (very dark, close to black)
...
class App {
  constructor() {
    ...
    this.colorSchemes = {
      1: {
        // Scheme 1: user-provided palette
        // Color 1: #050505
        // Color 2: #000000
        // Color 3: #D2E449
        // Color 4: #23240F
        // Color 5: #363530
        // Color 6: #000000
        color1: new THREE.Vector3(0.02, 0.02, 0.02),
        color2: new THREE.Vector3(0.0, 0.0, 0.0),
        color3: new THREE.Vector3(0.824, 0.894, 0.286),
        color4: new THREE.Vector3(0.137, 0.141, 0.059),
        color5: new THREE.Vector3(0.212, 0.208, 0.188),
  ...
  setColorScheme(scheme) {
    ...
    if (scheme === 1) {
      // Scheme 1: apply the six custom colors directly
    ...
    if (scheme === 1) {
      // Scheme 1: dark base using the provided near-black tones
      this.scene.background = new THREE.Color(0x000000); // Pure black
      uniforms.uDarkNavy.value.set(0.02, 0.02, 0.02); // #050505 as base
      ...
      // Neutral weights; color balance comes from the palette itself
  ...
  init() {
    ...
    // Desactivar interacción del fondo con el ratón / touch:
  ...
  onMouseMove(ev) {
    // Interacción desactivada: no añadimos trazas al touchTexture
    // para que el fondo no se deforme al mover el ratón.
    this.mouse = null;
  }
...
// Update color pickers from current scheme (si existen en el DOM)
...
// Update gradient when color picker changes (solo si existen)
for (let i = 1; i <= 6; i++) {
...
// Export all colors (solo si el botón existe)
...
if (exportAllBtn) {
  exportAllBtn.addEventListener("click", () => {
    ...
    for (let i = 1; i <= 6; i++) {
      const display = document.getElementById(`colorValue${i}`);
      if (display) colors.push(display.value);
    }
...
// Make cursor larger on hover over interactive elements (footer removed)
...
// Scroll indicator click → smooth scroll to target section
const scrollIndicator = document.querySelector(".scroll-indicator");
if (scrollIndicator) {
  scrollIndicator.addEventListener("click", () => {
    const targetSelector = scrollIndicator.dataset.target || "#sobre-mi";
